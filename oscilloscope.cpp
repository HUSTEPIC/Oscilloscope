...
void Oscilloscope::setupRealtimeDataDemo(QCustomPlot *customPlot) {
  ...
  */
  ...
  customPlot->graph(0)->setName("ch0");
  // customPlot->graph(0)->setBrush(QBrush(QColor(240, 255, 200)));
  // customPlot->graph(0)->setAntialiasedFill(false);
  customPlot->addGraph();  // red line
  customPlot->graph(1)->setPen(QPen(Qt::red));
  customPlot->graph(1)->setName("ch1");
...
void Oscilloscope::getData(const QVector <float> &data) {
...
void Oscilloscope::realtimeDataSlot() {
    ...
    // 添加数据1到曲线1
    if (!pauseEnabled)customPlot->graph(0)->addData(key, value0);
